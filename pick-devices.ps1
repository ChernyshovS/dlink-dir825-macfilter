...
function Update-View {
    ...
    try {
        ...
        foreach ($r in $inventory) {
            ...
            # Белый список — фильтр беспроводной сети, но запрещать галочку
            # проводному устройству нельзя. Настольный компьютер бывает и с
            # Wi-Fi, ноутбук — в док-станции, а запись пригодится заранее.
            # К тому же запрет получался односторонним: пока устройство было
            # выключено, тип подключения неизвестен и снять галочку удавалось,
            # а вернуть её после включения по кабелю — уже нет.
            if ($r.Wired -and -not $r.Wireless) {
                $row.Cells[$COL_WHITE].ToolTipText =
                    'Сейчас устройство подключено кабелем. Запись в белом списке подействует, когда оно подключится по Wi-Fi.'
            }
