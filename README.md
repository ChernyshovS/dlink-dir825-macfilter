...
Если `whitelist.json` ещё нет — например, проект только что скачан на другой компьютер, — окно берёт отметки прямо с роутера, а не считает список пустым. Иначе первое же «Применить» вызвало бы `sync` и стёрло с роутера всё, чего нет в свежей пустой таблице. Файл появится при первом сохранении или при включении списка; до тех пор внизу окна написано, откуда взяты отметки.
